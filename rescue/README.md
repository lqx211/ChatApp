...
- **零依赖**：不 require `api/config.php`、不 require `maintenance/` 或 `data/` 里任何代码；只用文件系统 + session + git 命令行。
...
| 升级 | 拉取 `origin/main` 覆盖代码（config/data/bkup/维护凭据保留，rescue 自身不动），完成后尽力重启 WSS |
| 降级 | 选任意历史 commit 回退（同样的排除项），完成后尽力重启 WSS |

危险操作（升级/降级）沿用维护面板的三重验证：
...
- `index.php` — 登录页 + 仪表盘/升级/降级三面板（自包含 CSS/JS）
- `lang.php` — English / 简体中文语言包
- `lib.php` — 凭据加载、自举（生成维护凭据）、DB 探测、git、WSS 重启等工具函数
- `worker.php` — 后台升级/降级执行器（nohup 运行，进度写 `data/rescue_progress.json`）
