  ...
  <div class="sidebar-nav" id="sidebarNavDefault">
   <div class="ng">
    ...
    <div class="ngb op" id="body-contactsGroup">
     ...
     <div class="na" onclick="openBotCreate()">🤖 <?php echo t('bot_add','添加机器人');?></div>
    </div>
   </div>
   <div class="ng">
   ...
   <div class="ng">
    ...
    <div class="ngb" id="body-appsGroup">
     <div class="na" onclick="switchPanel('deepseek')"><?php echo t('sb_deepseek', 'Deepseek');?></div>
...
<div class="main-content">
 ...
 <!-- DeepSeek AI 聊天（伪装成正常私聊；Key 存本地） -->
 <div class="panel" id="panel-deepseek">
  <div class="music-frame"><iframe data-src="../../apps/deepseek/index.php"></iframe></div>
 </div>
...
<div class="modal-overlay cl-modal" id="chatlogModal"><div class="modal-box"><h3 id="chatlogModalTitle"><?php echo t('cl_footer');?></h3><div id="chatlogModalBody" class="chatlog-bubbles"></div><div class="modal-actions"><button class="bsm" onclick="closeChatlogDetail()"><?php echo t('btn_cancel');?></button></div></div></div>
...
var SEND_RECEIPT=<?php echo (int)($currentUser['send_read_receipt'] ?? 1);?>;
var VIEW_RECEIPT=<?php echo (int)($currentUser['view_read_receipt'] ?? 1);?>;
...
<div class="modal-overlay" id="customDialog"><div class="modal-box cd-win"><div class="cd-titlebar"><span class="cd-title" id="cdTitle"></span><button type="button" class="cd-close" onclick="closeCustomDialog(false)" title="<?php echo t('btn_close','Close');?>"><img src="../../data/res/cil/cil-x.svg" style="width:14px;height:14px"></button></div><div class="cd-body"><p class="cd-msg" id="cdMsg"></p><input type="text" id="cdInput" class="cd-input" style="display:none"><div class="modal-actions cd-actions"><button class="bsm" id="cdCancel" onclick="cdResolve(false)"><?php echo t('btn_cancel','取消');?></button><button class="bsm" id="cdOk" onclick="cdResolve(true)"><?php echo t('btn_ok','OK');?></button></div></div></div></div>
...
<div class="modal-overlay" id="botCreateModal" onclick="botModalBackdrop(event,'botCreateModal')">
 <div class="modal-box ds-bot-modal">
  <h3>🤖 <?php echo t('bot_add','添加机器人');?></h3>
  <label class="ds-bot-label"><?php echo t('bot_kind_label','类型');?></label>
  <div class="ds-bot-kind">
   <label><input type="radio" name="botKind" value="normal" checked onchange="onBotKindChange()"> <?php echo t('bot_kind_normal','普通机器人');?></label>
   <label><input type="radio" name="botKind" value="pseudo" onchange="onBotKindChange()"> <?php echo t('bot_kind_pseudo','伪人（学习某个人的聊天记录）');?></label>
  </div>
  <div id="botPseudoBox" style="display:none">
   <label class="ds-bot-label" for="botTargetSelect"><?php echo t('bot_target_label','学习对象（要有一定聊天记录的人才学得出来）');?></label>
   <select id="botTargetSelect" class="ds-bot-input"><option value=""><?php echo t('bot_loading','加载中…');?></option></select>
  </div>
  <label class="ds-bot-label" for="botNameInput"><?php echo t('bot_name_label','名字');?></label>
  <input type="text" id="botNameInput" class="ds-bot-input" maxlength="24" placeholder="<?php echo t('bot_name_ph','给它起个名字，如 翻译官');?>" autocomplete="off">
  <div id="botTemplateWrap">
   <label class="ds-bot-label" for="botTemplate"><?php echo t('bot_template_label','模板（可选，会把对应人设填进下面）');?></label>
   <select id="botTemplate" class="ds-bot-input" onchange="applyBotTemplate()"><option value=""><?php echo t('bot_template_custom','自定义');?></option></select>
  </div>
  <div id="botPersonaWrap">
   <label class="ds-bot-label" for="botPersona"><?php echo t('bot_persona_label','人设（提示词）');?></label>
   <textarea id="botPersona" class="ds-bot-input ds-bot-textarea" placeholder="<?php echo t('bot_persona_ph','告诉它该怎么说话、擅长什么。例如：你是中英互译专家，用户发中文就译成地道英文，只给译文。');?>"></textarea>
  </div>
  <p class="ds-bot-note"><?php echo t('bot_note','机器人的回复用的是<strong>你自己的 DeepSeek API Key</strong>（在 <a href="/apps/deepseek/" target="_blank" rel="noopener">Deepseek 应用</a>的设置里填，服务端不保存）。<br>只能私聊、不能进群；删除这个联系人时会连聊天记录一起删掉。<br>伪人会先分析 ta 的聊天记录（断句、口头禅、表情包、心理画像），只存在你自己账号下，别人看不到。');?></p>
  <div class="ds-bot-actions">
   <button class="bsm" type="button" onclick="closeBotCreate()"><?php echo t('btn_cancel','取消');?></button>
   <button class="bsm ds-bot-primary" type="button" id="botCreateBtn" onclick="createBot()"><?php echo t('bot_create','创建');?></button>
  </div>
 </div>
</div>
...
<div class="modal-overlay" id="botOptModal" onclick="botModalBackdrop(event,'botOptModal')">
 <div class="modal-box opt-win">
  <div class="opt-head">
   <div class="opt-title" id="botOptTitle">🧠 <?php echo t('popt_title','人格优化器');?></div>
  ...
  <div class="opt-bar">
   <button class="bsm" type="button" onclick="optLoadEmojiQuestions(false)" title="<?php echo t('popt_btn_emoji_tip','扫一遍 ta 用过的表情，逐个问你含义');?>"><?php echo t('popt_btn_emoji','表情包待补全');?></button>
   <button class="bsm" type="button" onclick="optAskCheck()" title="<?php echo t('popt_btn_check_tip','让优化器通读人格，找问题');?>"><?php echo t('popt_btn_check','让优化器体检');?></button>
   <button class="bsm" type="button" onclick="optShowMemories()" title="<?php echo t('popt_btn_mem_tip','关于 ta 的事实（可手写/删除，聊天时会用上）');?>"><?php echo t('popt_mem_title','🧷 记忆点');?></button>
  </div>
  <div class="opt-input">
   <textarea id="botOptInput" rows="1" placeholder="<?php echo t('popt_input_ph','问优化器点什么…（Enter 发送，Shift+Enter 换行）');?>" onkeydown="optInputKey(event)"></textarea>
   <button class="bsm opt-send" type="button" onclick="optSend()"><?php echo t('popt_send','发送');?></button>
  </div>
  <p class="ds-bot-note" style="margin:6px 0 0"><?php echo t('popt_note','优化器用的是你的 DeepSeek API Key；这里的对话<strong>不会发给伪人</strong>，也不写进聊天记录。改动一律先给你看 diff。');?></p>
 </div>
</div>
