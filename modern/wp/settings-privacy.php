...
$sendReadReceipt = (int)($u['send_read_receipt'] ?? 1);
$viewReadReceipt = (int)($u['view_read_receipt'] ?? 1);
?>
<!DOCTYPE html>
<html lang="zh">
<head>
...
function toggleCol(col, swId, el) {
    api('toggle_' + col).then(function(d) {
        ...
        if (d.success) {
            ...
            // 发送已读回执：同步父页面 SEND_RECEIPT（与右键菜单同一开关）
            if (col === 'send_read_receipt' && window.parent) window.parent.SEND_RECEIPT = d[col];
            // 显示他人已读回执：同步父页面 VIEW_RECEIPT 并重绘本会话自己的回执标记
            if (col === 'view_read_receipt' && window.parent) {
                window.parent.VIEW_RECEIPT = d[col];
