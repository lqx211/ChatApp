 ...
 * perform: 三重验证 + 选目标 commit → git checkout 回退代码（排除 config/data/bkup、maintenance/config.php 与 rescue）
...
switch ($action) {
    ...
    case 'perform':
        ...
        // 回退代码（config/data/bkup 保留；maintenance/ 也会回退，除了机器相关的 config.php；rescue 永不回退）
        $coCmd = 'git checkout --force ' . escapeshellarg($target) . ' -- . \':!config\' \':!data\' \':!bkup\' \':!maintenance/config.php\' \':!rescue\'';
