...
switch ($action) {
    ...
    case 'discover':
        ...
        $stmt->execute($params);
        $users = $stmt->fetchAll();
        // 头像 URL：data/pp 优先 + mtime 版本号防缓存旧图
        foreach ($users as &$du) {
            $du['avatar'] = chatapp_avatar_url($du['avatar'] ?? null, $du['username'] ?? '', (int)($du['user_id'] ?? 0));
        }
        unset($du);
    ...
    case 'get_blocks':
        ...
        // 头像 URL：data/pp 优先 + mtime 版本号防缓存旧图
        foreach ($blocks as &$b) {
            $b['avatar'] = chatapp_avatar_url($b['avatar'] ?? null, $b['username'] ?? '', (int)($b['uid'] ?? 0));
        }
        unset($b);
    ...
    case 'upload_profile_bg':
        ...
        $pdo = db();
        // 封面存景字段（视频封面的位置/缩放）：pos_x/pos_y = 可视区中心百分比，zoom = 放大倍数，flip = 左右镜像
    ...
    case 'remove_profile_bg':
        ...
        $pdo->prepare("UPDATE users SET profile_bg_image = NULL, profile_bg_updated_at = NULL, bg_pos_x = 50, bg_pos_y = 0, bg_zoom = 1.00, bg_flip = 0 WHERE user_id = ?")->execute([$uid]);
        echo json_encode(['success' => true]);
        break;
