...
require_once __DIR__ . '/report_actions.php';   // 举报统一层（网页 + AI 工具 ca_report_user 共用）
...
switch ($action) {
    ...
    case 'submit':
        // 逻辑（含限流 / e2ee 证据 / 首次举报 EXP）已搬到 api/report_actions.php
        echo json_encode(report_action_submit(
            $pdo, $myUid, $myUsername,
            (string)($_POST['target'] ?? ''),
            (string)($_POST['reason'] ?? ''),
            (string)($_POST['message_ids'] ?? '[]'),
            (string)($_POST['decrypted'] ?? '{}')
        ));
        break;
