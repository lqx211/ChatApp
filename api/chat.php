...
switch ($action) {
    ...
    case 'all':
        ...
        $sel = "SELECT m.id, m.sender_id, su.username, su.display_name, su.avatar, su.user_id,
                       m.recipient_id, ru.username AS recipient_name, ru.display_name AS recipient_display, ru.user_id AS recipient_uid, m.read_at, m.receipt_visible,
                       m.message, m.msg_type, m.attachment, m.time, m.datetime, m.deleted_at, m.reply_to, m.temp_upload_id
                FROM messages m
                LEFT JOIN users su ON su.user_id = m.sender_id
                LEFT JOIN users ru ON ru.user_id = m.recipient_id";
...
function proc(array $msgs): array {
    ...
    foreach ($msgs as $m) {
        ...
        // 头像 URL：data/pp 优先（磁盘权威）+ mtime 版本号防浏览器缓存旧图；data URI 兜底
        ...
        if (!$m['is_deleted'] && !empty($m['attachment']) && !empty($m['msg_type'])) {
            if ($m['msg_type'] === 'file') {
                ...
                if ($m['temp_upload_id'] > 0) {
                    $tStmt = db()->prepare("SELECT revoked, expires_at, status FROM temp_uploads WHERE id = ?");
        ...
        // 已读回执（消息级快照）：receipt_visible NULL/1=显示，0=隐藏。
        // 隐藏 → 服务端直接抹掉已读时间，绝不下发（防破解：抓包/伪造接口也拿不到 read_at）。
