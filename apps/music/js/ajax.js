...
function in_array(needle, array, bool) {
    if (typeof needle == "string" || typeof needle == "number") {
        ...
        if(mkPlayer.debug) console.info("检查数组:" , array);
        for (var i in array) {
            if (needle == i) {
...
{
    // 已经有数据，直接回调
    if(mkPlayer.debug) console.info("AJAX音乐对象:",music);

    if(in_array("url",music) && music.url !== null && music.url !== "err" && music.url !== "") {
        callback(music);
        return true;
    }
    
    if(!in_array("id",music)) alert("FATAL ERROR");
    // id为空，赋值链接错误。直接回调
    if(music.id === null) {
        music.url = "err";
