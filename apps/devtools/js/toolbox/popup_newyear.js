...
var lib = {
	...
	'txtbox': function(height) {
		...
		if($(window).width() > 1400) {
			$("#develop_tools").width(1285);
			$(".span12").width(940);
			$('#develop_menu').width(1285);
			// 高分辨率屏幕下调整高度
			txt_height = $(window).height() - 300;
		} else {
			// 简式版工具栏显示
			$("#develop_tools").width(1233);
			$(".span12").width(840);
			$('#develop_menu').width(1233);
		}
		...
		if(height > 0) {
			...
			// 找到可用的高度参照物, 判断是否可以替换掉高分辨率值
			$('.accordion').each(function(i, data){
				if($(data).css('display') != 'none' && $(this).height() > txt_height) txt_height = $(this).height();
			});
		}
		
		// 设置最小高度
		if(txt_height < 400) txt_height = 400;

		$('.CodeMirror').height(txt_height).css('z-index', 3);
		$(".CodeMirror-gutters").height(txt_height);
	},
	// 提示信息框
	'popup': function() {
		//var tools_pos = $('.CodeMirror').position();
		var tools_width = $(window).width();
		var tools_height = $('.CodeMirror').height();
		$("#alert-msg").css('top', tools.top).css("margin-left", parseInt(tools_width/2)).css("top", parseInt(tools_height/2));
	},
...
var ONE = {
	...
	'urlencode' : function(type, charset) {
		if(!ONE.init(true)) return false;
		PUT.output(type == 1 ? encodeURIComponent(PUT.input()).replace(/(%20)/g,'+') : decodeURIComponent(PUT.input()).replace(/\+/g,' '));
	},
	...
	'hex' : function(type) {
		if(!ONE.init(true)) return false;
		var str_tmp = '';
		var put_data = PUT.input().replace(/[^0-9]+/g, '');
	...
	'regular': function() {
		func.openurl("page/regular.html");
	},
	'qrcode' : function() {
	...
	'qrcode_reader':function(){
		func.openurl("page/qrcodereader.html");
	},
	'contrast': function() {
		func.openurl("page/contrast.html");
	},
	'ua': function() {
		func.openurl("page/ua.html");
	},
	'lbs': function() {
	...
	<!--信息栏 开始-->
	'message': function() {
		var str_tmp = "您的IP是：[127.0.0.1] ";
		str_tmp += "[1]人在线";
		
		$("#ipaddr>code").text(str_tmp);
		delete str_tmp;
	}
	<!--信息栏 结束-->
}
